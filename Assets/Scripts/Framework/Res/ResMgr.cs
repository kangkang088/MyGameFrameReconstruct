...
public class ResInfo<T> : ResInfoBase
{
    ...
    //引用计数为零时是否需要立刻移除（为了避免一些大资源立刻移除造成的卡顿）
    public bool isDel;
...
public class ResMgr : BaseManager<ResMgr>
{
    ...
    {
        //移除不使用的资源之前，先把我们自己记录的那些引用计数为0，但没有移除记录的资源记录移除掉
    ...
    /// <summary>
    /// 清空字典，当我们加载资源时，不想使用对应的写在资源的方法时，我们可以在过场景或其他某个时刻调用这个方法
    /// </summary>
    /// <param name="callback">卸载完后的回调函数</param>
